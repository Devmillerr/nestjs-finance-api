...
`tsc --noEmit` sobre TODO el backend: los únicos errores son los enums que genera Prisma (`Role`, `PermissionName`, `PaymentMethod`, `PaymentStatus`, `ChargeType`, `ProductType`, `ContractStatus`), que desaparecen al ejecutar `prisma generate` — ningún error de lógica. ESLint limpio en todo el proyecto.

**Siguiente fase (Fase 3 del roadmap original):** idempotencia en creación de pagos/compras, logging estructurado, rate limiting ya está (Fase 1), documentación OpenAPI/Swagger, y validar `prisma generate` + `migrate` + tests contra una base de datos real antes de seguir agregando funcionalidad.
...
El schema completo (16 tablas, 7 enums, todos los índices de FK, RLS habilitado sin policies) está aplicado en el proyecto de Supabase (Postgres 17) directamente por SQL, sin pasar por `prisma migrate dev`.

**Verificado con los advisors de seguridad de Supabase:** cero errores/warnings de seguridad reales — solo avisos INFO de "RLS sin policies" (esperado y correcto: bloquea la API REST autogenerada de Supabase por defecto) y "índice sin uso" (esperado: la base está vacía, todavía no hay tráfico).

### Por qué no corras `prisma migrate dev` todavía

Como el schema se aplicó directo por SQL, Prisma no tiene su tabla `_prisma_migrations`. Si corres `migrate dev` ahora, Prisma va a pensar que faltan todas las tablas. La migración baseline está en `prisma/migrations/20260830000000_init/migration.sql` (el mismo SQL aplicado en Supabase) para marcarla como ya aplicada.

### Pasos para conectar el backend local
...
## Estado verificado contra la base real
...
- `schema.prisma`: faltaba la relación inversa `serviceContracts ServiceContract[]` en `User` — rompía `prisma generate`.
- `auth.service.ts`: `TokenPair` no estaba exportado.
- `invoices.service.ts`: tipado explícito reforzado en `resolveLines` (el resultado de `Promise.all` sobre product/service/serviceContract) — con los enums reales generados, la inferencia de tipos se comportaba distinto.
...
- Tabla `idempotency_keys` aplicada en Supabase, migración espejo en `prisma/migrations/20260831000000_add_idempotency_keys/`.
...
- **README.md** en inglés (decisión deliberada: un README en español reduce el alcance de quién lo lee antes de mirar el código).
- **SECURITY.md**: checklist honesto contra OWASP API Security Top 10, con lo que está resuelto y lo que no.
- **Bug encontrado al armar el checklist:** la API no estaba versionada (`/api/v1`) en la reescritura V3 — V2 sí lo tenía. Corregido en `main.ts` (`setGlobalPrefix`, con `/health` excluido a propósito).
...
- shadcn/ui armado a mano (`components.json`, `lib/utils.ts`, componentes `Button`/`Input`/`Label`/`Avatar`).
